<?php 
     include 'includes/_header.php';   
    
?>
                <div class="slider-fixed-container">
                    <div class="slider-fixed-frame">
                        <div class="rev-slider-full">
                            <div class="rev-slider-banner-full  rev-slider-full">
                                <ul>


                                    <li data-transition="fade" data-slotamount="7" data-masterspeed="300" >
                                        <img src="images/homepage/tutor1.png"  alt="rev-full1" data-fullwidthcentering="on">

                                        <div class="tp-caption slider-text-description sft str"  data-x="20" data-y="200" data-start="1000" data-easing="easeOutBack" data-end="4500" data-endspeed="500">
                                           <p style="color:white"> Making the abilty for a student to learn easier than ever</p>
                                           <p style="color:white"> Look for a tutor that can help you today</p> <br />
                                           <br />
                                            
                                        </div>
                                        <div class="tp-caption slider-text-description sft str"  data-x="20" data-y="280" data-start="1500" data-easing="easeOutBack" data-end="5000" data-endspeed="500">
                                            <a href="/Public/search.php" class="button btn-flat">Search Tutors and Classes</a>
                                        </div>                  

                                    </li>


                                    <li  data-transition="slideleft" data-slotamount="14">
                                        <img src="images/homepage/tutor2.png" alt="Rev Full">

                                      

                                        <div class="tp-caption slider-text-title sft str"
                                             data-x="20"
                                             data-y="150"
                                             data-speed="300"
                                             data-start="800"
                                             data-easing="easeOutCubic" data-end="6000" data-endspeed="500">
                                            <!--<a href="#" class="button btn-flat">Our Motto</a>-->
                                        </div>


                                        <div class="tp-caption slider-text-description sft str"  data-x="20" data-y="200" data-start="1000" data-easing="easeOutBack" data-end="4500" data-endspeed="500">
                                           <p style="color:white">Tutoring Should be about more then just paper work</p>
                                            
                                        </div>
                                        <div class="tp-caption slider-text-description sft str"  data-x="20" data-y="280" data-start="1500" data-easing="easeOutBack" data-end="5000" data-endspeed="500">
                                            <a href="/Admin/login.php" class="button btn-flat">Tutor/Admin Log In</a>
                                        </div>                

                                    </li>

                                </ul>
                                <div class="tp-bannertimer tp-bottom"></div>
                            </div>
                        </div>        </div>
                </div>

            </div><!--.top wrapper end -->

            <div class="loading-container">
                <div class="spinner">
                    <div class="double-bounce1"></div>
                    <div class="double-bounce2"></div>
                </div>
            </div>


            <div class="content-wrapper hide-until-loading"><div class="section-content top-body">
                    <div class="container">
                        <div class="space-sep20"></div>
                        <div class="row">
                            <div class="col-md-4 col-sm-4"> 
                                <div class="content-box style5 small  animated"
                                     data-animtype="fadeIn"
                                     data-animrepeat="0"
                                     data-animspeed="1s"
                                     data-animdelay="0.2s"
                                     >
                                    <h4 class="h4-body-title">
                                        <i class=""></i>
                                        <strong>Students </strong> 
                                    </h4>
                                    <div class="content-box-text">
                                         If you are a student looking to be tutored you can use the Tutor/Class Search button<br />After going to the Tutor/Class Search Page:<br />
                <ul>
                    <li>You can Search for a tutor name to see when they will be teaching and where</li>
                    <li>You can Search for different classes you might need help in</li>
                    <li>You can Search for key terms for tutors with information in a subject you are looking for</li>
                </ul>

                                    </div>

                                </div>
                            </div>
                            <div class="col-md-4 col-sm-4"> 
                                <div class="content-box style5 small  animated"
                                     data-animtype="fadeIn"
                                     data-animrepeat="0"
                                     data-animspeed="1s"
                                     data-animdelay="0.2s"
                                     >
                                    <h4 class="h4-body-title">
                                        <i class=""></i>
                                        <strong>Tutor </strong> 
                                    </h4>
                                    <div class="content-box-text">
                                       If you are a tutor you can use the Log in page in the top right corner<br />After Logging in:<br />
                <ul>
                    <li>You can use the Clock In and Clock Out functions</li>
                    <li>You can add information about students you tutored</li>
                    <li>You can change your availability</li>
                    <li>You can change their description</li>
                    <li>You can edit your daily notification</li>
                </ul>

                                    </div>

                                </div>
                            </div>
                            <div class="col-md-4 col-sm-4"> 
                               
                            </div>


                        </div>
                        <div class="space-sep20"></div>



                    </div>    
                </div>
               

                    <div class="space-sep40"></div>   

                    <div class="space-sep40"></div>

                    
                
            </div><!--.content-wrapper end -->
<?php
include 'includes/_footer.php';
?>

